...
def setup_logging(log_level: str = "ERROR"):
	...
	handler = RotatingFileHandler("bot_logs.log", maxBytes=5_000_000, backupCount=3, encoding="utf-8")
...
def parse_args():
	...
	parser.add_argument(
		"--lazy-imports",
		action="store_true",
		help="Не загружать все команды/события при старте, загружать по требованию"
	)
...
if __name__ == "__main__":
	...
	# Загружаем все импорты динамически (если не включён lazy-режим)
	if not args.lazy_imports:
		load_all_imports()
