...
async def tweak_permission_command(ctx, username: str, title: str, permission: str, server: str = "mrp"):
    if not await ss14_db.get_admin_permission(username, db_name=server):
        await ctx.send(f"Пользователь {username} не имеет прав на {server.upper()}")
        return
