...
async def create_admin_info_embed():
    """
    Создает эмбед с информацией о сервере SS14.
    Возвращает кортеж (success, embed) где success - булево значение успеха операции.
    """
    url = f"http://{ADDRESS_MRP}:1212/admin/info"
    
    try:
        response = requests.get(url, headers=POST_ADMIN_HEADERS, timeout=10)
        ...
        if response.status_code == 200:
            ...
            error_embed = disnake.Embed(
                ...
                description=f"Не удалось получить данные с сервера.\nКод ошибки: {response.status_code}",
        ...
        error_embed = disnake.Embed(
            ...
            description=f"Не удалось подключиться к серверу: {str(e)}",
            color=disnake.Color.red()
        )
        return False, error_embed
        ...
        error_embed = disnake.Embed(
            ...
            description=f"Произошла ошибка при получении данных: {str(e)}",
            color=disnake.Color.red()
        )
        return False, error_embed
